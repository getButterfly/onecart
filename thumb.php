...
if (!defined('FILE_CACHE_SUFFIX'))
    define('FILE_CACHE_SUFFIX', '.imagecache.txt');
if (!defined('FILE_CACHE_PREFIX'))
    define('FILE_CACHE_PREFIX', 'imagecache');
...
class ImageCache {
    ...
    public static function start(){
        $squeeze = new ImageCache();
        $squeeze->handleErrors();
        $squeeze->securityChecks();
        if($squeeze->tryBrowserCache()){
            exit(0);
        }
        $squeeze->handleErrors();
        if($squeeze->tryServerCache()){
            exit(0);
        }
        $squeeze->handleErrors();
        $squeeze->run();
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/imagecache_cacheLastCleanTime.touch';
		
		//If this is a new ImageCache installation we need to create the file
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$imgType = "";
		$tempfile = tempnam($this->cacheDirectory, 'imagecache_tmpimg_');
		...
		$tempfile4 = tempnam($this->cacheDirectory, 'imagecache_tmpimg_');
	...
	protected function getLocalImagePath($src){
		...
		if(! $this->docRoot){
			...
			return $this->error("Could not find your website document root and the file specified doesn't exist in imagecaches directory. We don't support serving files outside imagecache's directory without a document root for security reasons.");
	...
	protected function serveExternalImage(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'imagecache');
	...
	protected function serveCacheFile(){
		if(! is_file($this->cachefile)){
			$this->error("serveCacheFile called in ImageCache but we couldn't find the cached file.");
			return false;
		}
	...
	protected function getURL($url, $tempfile){
		...
		if (function_exists('curl_init')) {
			...
			curl_setopt ($curl, CURLOPT_WRITEFUNCTION, 'ImageCache::curlWrite');
